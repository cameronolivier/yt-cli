...
import ora from 'ora';
import { getVideoInfo, downloadVideo as downloadVideoFile, downloadTranscripts } from '../utils/youtube';
...
export async function downloadVideo(url: string, options: CommandOptions): Promise<void> {
  ...
  try {
    ...
    spinner.succeed(`Found video: ${chalk.cyan(videoInfo.title)} by ${chalk.yellow(videoInfo.uploader)}`);
    
    console.log(chalk.gray(`Duration: ${Math.floor(videoInfo.duration / 60)}:${(videoInfo.duration % 60).toString().padStart(2, '0')}`));
    ...
    try {
      const downloadResult = await downloadVideoFile(url, {
        outputDir,
      ...
      videoFilePath = await findDownloadedVideoFile(outputDir, videoInfo.title, videoInfo.id, options.audioOnly);
      if (videoFilePath) {
        spinner.succeed(options.audioOnly ? 'Audio downloaded' : `Video downloaded: ${chalk.green(path.basename(videoFilePath))}`);
      } else {
    ...
    if (options.transcript) {
      ...
      try {
        ...
        spinner.warn(`Transcript download failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (!options.audioOnly && options.compression && videoFilePath) {
      ...
      try {
        ...
        console.error(chalk.red('Compression error:'), error instanceof Error ? error.message : 'Unknown error');
      }
    }
    ...
    if (videoFilePath) {
      ...
      console.log(chalk.gray(`Video: ${path.basename(videoFilePath)} (${formatFileSize(finalInfo.size)})`));
    }
...
async function findDownloadedVideoFile(outputDir: string, title: string, videoId: string, audioOnly: boolean = false): Promise<string | undefined> {
  try {
...
async function organizeTranscripts(transcriptFiles: string[]): Promise<Record<string, TranscriptFiles>> {
